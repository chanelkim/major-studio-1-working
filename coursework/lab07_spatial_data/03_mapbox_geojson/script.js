...
d3.json("africa.geo.json").then((geojson) => {
  ...
  let featureElement = svg
    ...
    .on("mouseover", function (d) {
      ...
      d3.select(this).attr("fill", "dodgerblue"); //IN-CLASS: works in tandem with "mousemove"
      //we control name
      d3.select("#hover").text(
        d.srcElement.__data__.properties.name +
          " has a population of " +
          (d.srcElement.__data__.properties.pop_est / 1000000).toFixed(1) +
          " Mio." //IN-CLASS: must be in the format that the geoJSON offers
      );
  ...
  svg.append("text").attr("id", "hover");

  // sync map views and scales on reset
  let update = () => {
    featureElement.attr("d", path); //IN-CLASS: where we inform mapbox of the new "path" state, featureElement is the name of the variable, informs the scaled svg map
  };
